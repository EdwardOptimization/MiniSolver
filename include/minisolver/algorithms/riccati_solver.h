...
public:
    ...
    // Iterative Refinement Implementation (RESTORED for Split Architecture)
    ...
    bool refine(TrajectoryType& traj, const TrajectoryType& original_system, int N, double /*mu*/, double /*reg*/, const SolverConfig& config) override {
        ...
        // In new architecture: workspace contains dx/du, model contains A/B/f_resid
        auto* workspace = traj.get_workspace();
        auto* state = traj.get_active_state();
        auto* model = original_system.get_model_data();
        ...
        for(int k=0; k<N; ++k) {
            ...
            // delta_u = K * delta_x
            delta_u.noalias() = workspace[k].K * delta_x;
            
            // 2. Compute Dynamic Defect of the current solution
            // Reconstruct the affine term (linearization defect)
            auto* orig_state = original_system.get_active_state();
            MSVec<double, Model::NX> linearization_defect = model[k].f_resid - orig_state[k+1].x;
            
            MSVec<double, Model::NX> predicted_dx_next;
            predicted_dx_next.noalias() = model[k].A * workspace[k].dx;
            predicted_dx_next.noalias() += model[k].B * workspace[k].du;
            predicted_dx_next += linearization_defect;
            
            MSVec<double, Model::NX> error = predicted_dx_next - workspace[k+1].dx;
            ...
            delta_x_next.noalias() = model[k].A * delta_x;
            delta_x_next.noalias() += model[k].B * delta_u;
            delta_x_next += error;
            
            // 4. Apply to Trajectory
            workspace[k].dx += delta_x;
            workspace[k].du += delta_u;
            
            delta_x = delta_x_next;
        }
        
        // Apply last state correction
        workspace[N].dx += delta_x;
