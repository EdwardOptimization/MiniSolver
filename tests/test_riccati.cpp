...
TEST(RiccatiTest, NonSPDQuuFreezesControlDimsInsteadOfFailing)
{
    ...
    LinearSolveResult result
        = solver.solve(traj, N, 0.01, 1e-9, InertiaStrategy::REGULARIZATION, config);
    EXPECT_TRUE(result.ok);
    EXPECT_TRUE(result.degraded_step)
        << "Small-NU fallback freezes a control dimension and must be externally visible";
    EXPECT_EQ(result.degraded_riccati_freeze_count, N);
...
TEST(RiccatiTest, TrivialLQR)
{
    ...
    // So we set traj[0].x = [0...], but actually Riccati linearizes around current traj.
    ...
    traj[N].q(0) = 1.0; // Gradient at terminal cost
    ...
    EXPECT_NEAR(traj[1].K(0, 0), -0.5, 1e-5);
    EXPECT_NEAR(traj[1].K(1, 1), -0.5, 1e-5);
