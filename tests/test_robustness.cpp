...
// If regularization fails, the KKT system [Q  A^T; A  0] will be singular if Q=0 and A is not full
// rank (here A=[1] is full rank, so KKT is invertible, but pure Newton on Cost would fail). Let's
// make it harder: Q=0, and unconstrained degrees of freedom. Min 0*x^2 + 0*u^2 s.t. x_{k+1} = x_k +
// u_k x_0 = 0 No other constraints. Solution is x=0, u=0 (or any u if cost truly 0). But Riccati
// backup requires inverting (R + B^T P B). If R=0 and P=0 (terminal), this is singular. The solver
// should add regularization to R to proceed.
...
TEST(RobustnessTest, SingularHessianRecovery)
{
    ...
    // Note: If reg_init (1e-8) is sufficient to make it invertible numerically (machine epsilon
    // 1e-16), it might not need to increase if singular_threshold is loose. However, since R=0,
    // eigenvalues are all 0 (or small noise). The Riccati solver should bump reg if it encounters
    // small pivots.
