...
TEST(SolverQualityTest, FiniteDifferenceJacobianVerification)
{
    ...
    for (int j = 0; j < NX; ++j) {
        ...
        for (int i = 0; i < NC; ++i) {
            ...
            EXPECT_NEAR(C_an(i, j), fd, 1e-4) << "C(" << i << "," << j << ") mismatch";
        }
    }
    ...
    for (int j = 0; j < NU; ++j) {
        ...
        for (int i = 0; i < NC; ++i) {
            double fd = (kp_p.g_val(i) - kp_m.g_val(i)) / (2 * eps);
            EXPECT_NEAR(D_an(i, j), fd, 1e-4) << "D(" << i << "," << j << ") mismatch";
        }
    }
...
TEST(SolverQualityTest, KKTOptimalityConditions)
{
    ...
    for (int k = 0; k <= N; ++k) {
        ...
        for (int i = 0; i < 1; ++i) {
            ...
            EXPECT_LT(comp, comp_tol)
                << "Complementarity violated at k=" << k << ": s*lam=" << comp;
        }
    }

    // (d) Dynamics feasibility: x_{k+1} ≈ f(x_k, u_k)
    for (int k = 0; k < N; ++k) {
        double defect = std::abs(
            solver.get_state(k + 1, 0) - (solver.get_state(k, 0) + solver.get_control(k, 0) * 0.1));
...
TEST(SolverQualityTest, AnalyticalSolutionUnconstrained)
{
    ...
    for (int k = 1; k <= N; ++k) {
        ...
        EXPECT_LE(abs_x, prev_abs_x + 1e-6) << "State should decrease monotonically at k=" << k;
        prev_abs_x = abs_x;
    }
...
TEST(SolverQualityTest, MehrotraConvergenceAdvantage)
{
    ...
    // Solve with Monotone
    MiniSolver<CarModel, 20> solver_mono(
        N, Backend::CPU_SERIAL, make_config(BarrierStrategy::MONOTONE));
    setup_solver(solver_mono);
    SolverStatus status_mono = solver_mono.solve();
    int iters_mono = solver_mono.get_iteration_count();

    // Solve with Mehrotra
    MiniSolver<CarModel, 20> solver_meh(
        N, Backend::CPU_SERIAL, make_config(BarrierStrategy::MEHROTRA));
    ...
    EXPECT_LE(iters_meh, iters_mono)
        << "Mehrotra (" << iters_meh << " iters) should be faster than Monotone (" << iters_mono
        << " iters)";
