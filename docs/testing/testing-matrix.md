...
| Riccati robustness diagnostics | `test_riccati_robust` pins the contract: (1) default config uses `RiccatiRobustMode::STANDARD`; (2) `SolverInfo::riccati_indefinite_blocks` and `SolverInfo::riccati_max_diagonal_perturbation` stay at zero on a clean SPD problem in both `STANDARD` and `INERTIA_AWARE_DIAGNOSTICS` modes; (3) `INERTIA_AWARE_DIAGNOSTICS` does not flip `degraded_step` when no inertia correction occurred; (4) `SolverInfo::reset()` clears both counters; (5) config validation accepts both modes and rejects unknown enum values. Square-root and `FACTORIZATION_MODIFY` paths remain explicit non-goals (see OD-006 and `solver-capability-adoption-plan.md` P1) |
