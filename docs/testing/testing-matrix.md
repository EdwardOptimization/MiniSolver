...
| Pareto-frontier filter pruning | `test_filter_pareto` and the rewritten `test_line_search.FilterHistoryParetoCollapsesMonotonicallyImprovingSequence` pin the contract: (1) on a strictly improving (theta, phi) sequence, the filter history collapses to a single Pareto entry (`MonotonicallyImprovingMetricsCollapseFrontierToSingleEntry`); (2) on a strictly worsening sequence every new entry is rejected as redundant and the seed entry is preserved (`StrictlyWorseningSequenceMarksEveryNewEntryRedundant`); (3) `LineSearchResult::filter_entries_pruned` reports the number of dominated entries removed during insertion, `filter_redundant_inserts` reports a failed insert when the new entry is itself dominated, and `filter_size_after` mirrors the live frontier size; (4) `MeritLineSearch` leaves all three filter fields at zero; (5) `SolverInfo::filter_entries_pruned_total` / `filter_redundant_inserts_total` / `filter_max_history_size` accumulate across an entire solve and are cleared by `SolverInfo::reset()`; (6) the gamma_phi-aware dominance contract is pinned by `NonZeroGammaPhiPreservesParetoIncomparableEntry`, `NonZeroGammaPhiPrunesEntriesDominatedInPsiSpace`, and `ZeroGammaPhiBehavesAsPlainPareto` - dominance is checked in (theta, psi) space with `psi = phi - gamma_phi * theta`, so plain (theta, phi) Pareto cannot silently widen the forbidden region when gamma_phi > 0 |
