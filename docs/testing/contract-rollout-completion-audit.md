...
does not mean every future `P1/P2` replay, benchmark, or nightly item is already
implemented; it means those rows have owners and explicit evidence paths, while
all current `P0` contract rows are covered.
...
| `P1` rows are either covered or have an owner and clear deferred evidence path. | Each remaining `partial` row in `contract-coverage-matrix.md` has an owner module and a `Deferred path:` note. | Complete |
...
Remaining `partial` rows are not blockers for the initial rollout. They become
blocking only when a future behavior change touches that contract, or when a
release scope explicitly promotes the row to `P0`.
