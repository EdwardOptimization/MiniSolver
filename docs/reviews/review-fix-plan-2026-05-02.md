...
| Post-line-search early stop uses pre-line-search residual snapshot. | fixed | Covered by `BarrierResidualContractTest.PostsolveRechecksPrimalResidualAfterLoopOptimal`; the old private helper has been removed. | MiniSolver no longer certifies `OPTIMAL` immediately after line search. The next iteration or `postsolve()` evaluates fresh residuals on a consistent iterate. |
| Post-line-search early stop still combines accepted primal data with stale pre-line-search dual infeasibility. | fixed | Covered by `BarrierResidualContractTest.PostsolveRechecksDualResidualAfterLoopOptimal`; the old private helper has been removed. | MiniSolver no longer certifies `OPTIMAL` immediately after line search. The next iteration or `postsolve()` evaluates fresh residuals on a consistent iterate. |
