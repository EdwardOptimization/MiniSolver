...
| **N-API-1** Seven user-facing setter APIs are silent-return or warning-return on bad input. | deferred-design | [`solver.h`](../../include/minisolver/solver/solver.h) high-level API setters; [`api-logger-boundary-design.md`](../architecture/api-logger-boundary-design.md). | Design contract accepted: convert setters to explicit `ApiStatus` returns, add checked scalar getter overloads where `0.0` is ambiguous, avoid `last_api_status()` side channels and duplicate `try_set_*` surfaces. Implementation must proceed by red tests per setter family. |
...
| **N-PREC-2** Tolerance fields' physical meaning not documented. | partly-fixed | [`scaling-normalization-design.md`](../architecture/scaling-normalization-design.md) now defines scaled vs unscaled tolerance semantics and documents that `SolverInfo::primal_inf` is the internal metric while `unscaled_primal_inf` reports physical constraint residuals. [`solver_options.h`](../../include/minisolver/core/solver_options.h) still has only brief field comments. | Add short inline comments in `SolverConfig` after the scaling contract stabilizes; keep detailed semantics in the architecture doc. |
