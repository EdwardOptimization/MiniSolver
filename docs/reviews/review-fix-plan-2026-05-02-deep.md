...
| **N-OBS-1** `SolverContext` exposes ~8 of ~20 capability-adoption-plan diagnostic fields. Cannot query degraded-fallback flag, SOC counts, restoration counts, scaling status, linear-solver factorization status, etc. | partly-fixed | [`types.h`](../../include/minisolver/core/types.h), [`solver.h`](../../include/minisolver/solver/solver.h), [`test_status.cpp`](../../tests/test_status.cpp), [`test_bugfixes.cpp`](../../tests/test_bugfixes.cpp). | First batch added fixed-size `SolverInfo` counters for degraded Riccati freeze stages, regularization escalations, restoration attempts, and restoration successes. SOC counts remain deferred until line search returns an explicit result object instead of only `alpha`. |
...
1. **N-OBS-1**: continue structured diagnostics. First batch is done for
   degraded Riccati freeze stages, regularization escalations, and restoration
   attempt/success counts. Remaining diagnostics need explicit phase contracts:
   SOC counts require a line-search result object; scaling status waits for
   N-MOD-2; factorization status waits for linear-solver result expansion.
