...
| **N-DEG-1** Riccati small-NU freeze produces silent zero-control direction; user receives "OPTIMAL" status with frozen `du[i] = 0`. | fixed | [`riccati.h`](../../include/minisolver/solver/riccati.h), [`riccati_solver.h`](../../include/minisolver/algorithms/riccati_solver.h), [`solver.h`](../../include/minisolver/solver/solver.h), [`test_riccati.cpp`](../../tests/test_riccati.cpp), [`test_status.cpp`](../../tests/test_status.cpp). | Fixed by surfacing the freeze fallback through `RiccatiSolver::last_solve_degraded()` and `SolverInfo::degraded_step`. A focused Riccati test and end-to-end `SolverInfo` test cover the diagnostic. A detailed per-event counter remains part of the broader diagnostics backlog under N-OBS-1. |
