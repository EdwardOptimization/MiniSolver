...
EvalResult evaluate_config(const TunableParams& params)
{
    ...
    // 3. Bad Initial Guess (Cold start with zero inputs is already "bad", but let's try non-zero
    // bad inputs) Actually, solver.rollout_dynamics() overwrites state guesses, so we only control
    // control guesses.
    scenarios.push_back({ "BadGuess", [&](MiniSolver<CarModel, 100>& s) {
                             ...
                             for (int k = 0; k <= horizon; ++k) {
                                 ...
                                 if (k < horizon) {
                                     // Initialize with "Turn Left" guess which is wrong for
                                     // obstacle at (12,0)
