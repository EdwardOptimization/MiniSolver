...
template<typename MatrixType>
void fill_spd_1x1(MatrixType& A, XorShift64& rng) {
    A(0, 0) = rng.uniform(0.5, 3.0);
}

template<typename MatrixType>
void fill_spd_2x2(MatrixType& A, XorShift64& rng) {
...
template<typename MatrixType>
void fill_spd_3x3(MatrixType& A, XorShift64& rng) {
...
BenchArgs parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (s == "--iters") {
            ...
            if (v) a.iters = std::stoll(v);
        } else if (s == "--mats") {
            const char* v = next_val(i);
            if (v) a.mats = std::stoi(v);
        } else if (s == "--repeats") {
            const char* v = next_val(i);
            if (v) a.repeats = std::stoi(v);
        } else if (s == "--epsilon") {
            const char* v = next_val(i);
            if (v) a.epsilon = std::stod(v);
        } else if (s == "--help" || s == "-h") {
            std::cout
...
double time_inv_ns_per_call(const BenchArgs& args, FillFn fill, InvFn inv_fn) {
    ...
    if (sink == 123456.789) std::cerr << "sink=" << sink << "\n";
    return best_ns;
}
...
void bench_case(const BenchArgs& args, const std::string& label, FillFn fill_fn) {
    const double legacy_ns = time_inv_ns_per_call<MatrixType>(
        args,
        fill_fn,
        [](const MatrixType& A, MatrixType& A_inv, double eps) { return legacy_fast_inverse(A, A_inv, eps); });

    const double new_ns = time_inv_ns_per_call<MatrixType>(
        args,
        fill_fn,
        [](const MatrixType& A, MatrixType& A_inv, double eps) { return minisolver::fast_inverse(A, A_inv, eps); });

    const double delta_pct = (new_ns - legacy_ns) / legacy_ns * 100.0;

    std::cout << std::left << std::setw(10) << label
              << " legacy " << std::fixed << std::setprecision(3) << legacy_ns << " ns/call"
...
int main(int argc, char** argv) {
    ...
    bench_case<minisolver::MSMat<double, 1, 1>>(args, "1x1 SPD", fill_spd_1x1<minisolver::MSMat<double, 1, 1>>);
    bench_case<minisolver::MSMat<double, 2, 2>>(args, "2x2 SPD", fill_spd_2x2<minisolver::MSMat<double, 2, 2>>);
    bench_case<minisolver::MSMat<double, 3, 3>>(args, "3x3 SPD", fill_spd_3x3<minisolver::MSMat<double, 3, 3>>);
