...
template <int N>
__host__ __device__ GpuLinearOp<N> combine_ops(const GpuLinearOp<N>& b, const GpuLinearOp<N>& a)
{
    ...
    // Order matters: op(b, a) usually means applying 'b' then 'a' (or similar, depending on
    // convention).
    GpuLinearOp<N> res;
...
// MPX Functor
template <int N> struct MPXOp {
    __host__ __device__ GpuLinearOp<N> operator()(
        const GpuLinearOp<N>& a, const GpuLinearOp<N>& b) const
    {
...
template <int NX> void gpu_dispatch_solve(std::vector<GpuLinearOp<NX>>& h_ops, Backend mode)
{
    int N = h_ops.size();
