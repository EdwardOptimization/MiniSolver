...
void save_trajectory_csv(const std::string& filename, const SolverType& solver,
    const std::vector<double>& dts, double obs_x, double obs_y, double obs_rad)
{
    ...
    for (int k = 0; k <= horizon; ++k) {
        ...
        file << current_t << "," << x[0] << "," << x[1] << "," << x[2] << "," << x[3] << "," << u0
             << "," << u1 << "," << g_obs << "," << obs_x << "," << obs_y << "," << obs_rad << "\n";
    }
    file.close();
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    // Pure IPM often benefits from a consistent initial rollout (or not, but let's keep it for fair
    // comparison)
